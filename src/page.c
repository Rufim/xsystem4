...
void variable_fini(union vm_value v, enum ain_data_type type, bool call_dtor)
{
	...
	case AIN_REF_TYPE:
	/*
	 * ★★ЗДЕСЬ КОРЕНЬ «мёртвых экранов», но правка НЕ сводится к `case AIN_IFACE`.
	 *
	 * Факт: у нас НЕ УМИРАЕТ НИ ОДНА СЦЕНА. `Scenes::RunIScene` держит сцену в
	 * локальной `s` типа `IScene` и удаляет через `.LOCALDELETE s`; замер
	 * `XSYS4_STRUCT_WATCH` показал, что у `TitleScene`, `title::CActivity` и
	 * `title::CScreen` refcount не опускается ниже 1, деструкторы `@1` не зовутся
	 * ни разу, а `title::CScreen@UpdateEvent` тикает уже ПОСЛЕ завершения
	 * `RunIScene` титула (в стеке пролога `タイトル` нет) и каждый кадр воскрешает
	 * свой освобождённый парт — плёночный шум в углу пролога.
	 *
	 * ЗАМЕР ПРАВКИ (проведён): добавить AIN_IFACE сюда + `heap_ref` в `vm_copy` +
	 * не освобождать одолженные аргументы в ветке «делегат завершён» — сцены
	 * НАЧИНАЮТ УМИРАТЬ (вызвался `CAliceLogoScene@1`) и ПЛЁНКА ИСЧЕЗАЕТ с экрана
	 * (0 видимых партов с её CG в дампе). Но дальше падает `double free` в
	 * `activity::detail::CUserComponentManager@SetComponentImp`: там интерфейс
	 ...
	 * ЧТО ЕЩЁ ПРОВЕРЕНО (все пути владения у игры В ПОРЯДКЕ, ссылку берут):
	 *  • HLL `Array.PushBack` берёт `heap_ref` для объектного элемента
	 *    (`ix_elem_is_object`), и wrap<интерфейс> кладётся двумя слотами;
	 *  • сеттер интерфейсного ПОЛЯ (`CUserComponentSet@UserComponent::set`)
	 *    освобождает старое значение (`DELETE`) и берёт своё (`SP_INC`);
	 *  • `CUserComponentSet@Set` раздаёт значения именно через эти сеттеры.
	 *
	 * ★ГЛАВНОЕ: падение с этим `case` НЕДЕТЕРМИНИРОВАНО. Под трейсом освобождений
	 * (`XSYS4_UNREF_TRACE=1`, он же замедляет исполнение) прогон проходит ЧИСТО —
	 * 0 double free; без трейса падает. Значит расходится не статическая модель
	 * владения, а ПОРЯДОК освобождения внутри кадра: похоже, значение уже
	 * освобождено кем-то в этом же кадре, а слот переменной не обнулён.
	 *
	 * ★ВАЖНОЕ УТОЧНЕНИЕ (снимает прежнее сомнение): освобождать интерфейсы здесь
	 * ДОЛЖНО. В `Scenes::RunIScene` реальная инструкция `DELETE` ровно ОДНА (она
	 * освобождает старое значение dummy перед присваиванием), а `.LOCALDELETE` —
	 * это МАРКЕР дизассемблера alice, а не инструкция: проверено сырым дампом
	 * (`alice ain dump -C`, в теле функции 0 DELETE-инструкций против 4
	 * «DELETE» в обычном дампе). Значит освобождение локальных при выходе —
	 * обязанность ДВИЖКА, и арифметика с этим `case` сходится: alloc(1) +
	 * SP_INC(1) − fini(dummy) − fini(s) = 0, сцена умирает.
	 *
	 * ★ГДЕ ЛОМАЕТСЯ (следующий шаг, кандидат назван): падение приходит при
	 * освобождении GENERIC-контейнера Ixseal (`variable_fini` type=**79**,
	 * AIN_ARRAY) на выходе из `Scenes::RunIScene` — это видно в сообщении о
	 * double free, оно теперь печатает тип и место последнего `variable_fini`.
	 * Освобождается слот 7487, а падает СОСЕДНИЙ 7486 — характерный признак того,
	 * что «базу интерфейса» приняли за объект. И правда:
	 * `array_handle_elem_type()` НЕ включает AIN_ARRAY (79), поэтому у generic
	 * контейнера с интерфейсными элементами ширина элемента считается 1 слотом
	 * вместо 2, и владеющим оказывается не тот слот. Проверять надо это, а не
	 * `variable_fini`.
	 *
	 * Диагностика: `heap_double_free` печатает стек вызовов игры,
	 * `XSYS4_FINI_TRACE=<функция>` — какие типы освобождаются при её выходе,
	 * `XSYS4_UNREF_TRACE=1` — каждое освобождение с типом и функцией.
	 */
		if (v.i == -1)
			break;
		...
		// Последнее освобождение — чтобы сообщение о double free называло ТИП и
		// функцию, а не только номер слота (слоты переиспользуются, и по номеру
		// виновника не опознать).
		vm_note_last_fini(type, v.i, vm_current_function_name());
		if (call_dtor)
			heap_unref(v.i);
		else
			exit_unref(v.i);
