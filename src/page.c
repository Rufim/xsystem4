...
void variable_fini(union vm_value v, enum ain_data_type type, bool call_dtor)
{
	 ...
	 *
	 * Подсказка рядом, в `variable_initval_var`: игра, освободив временный var
	 * типа 89, САМА пишет туда `PUSH -1; X_ASSIGN 1`. То есть после `DELETE`
	 * интерфейсный слот должен становиться -1; если инструкция `DELETE` этого не
	 * делает, выход из функции освобождает то же значение ВТОРОЙ раз. Это и надо
	 * проверять следующим — обнуление слота в `DELETE`, а не владение в fini.
	 *
