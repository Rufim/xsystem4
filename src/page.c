...
void variable_fini(union vm_value v, enum ain_data_type type, bool call_dtor)
{
	...
	case AIN_REF_TYPE:
	case AIN_IFACE:
	// ★`wrap<структура>` (AIN_WRAP) — тоже ВЛАДЕЮЩИЙ хэндл: `variable_initval_var`
	// инициализирует его null-маркером -1, как объект. Пока его тут не было, поля
	// вида `wrap<title::CActivity> <Activity>` у `TitleScene` не освобождались —
	// сцена умирала, а её активность и экран (`title::CScreen`) жили дальше и
	// тикали покадровым событием.
	case AIN_WRAP:
	/*
	 * ★★ВЛАДЕЮЩИЕ ХЭНДЛЫ: `ref <интерфейс>` (AIN_IFACE, 89) и `wrap<структура>`
	 * (AIN_WRAP). Оба владеют heap-слотом объекта и обязаны его освобождать —
	 * `variable_initval_var` инициализирует их null-маркером -1, как объект, а
	 * `slot_owns_heap_ref()` в vm.c считает 89 владельцем и берёт `heap_ref`.
	 *
	 * Пока их здесь не было, у нас НЕ УМИРАЛА НИ ОДНА СЦЕНА: `Scenes::RunIScene`
	 * держит сцену в локальной `s` типа `IScene`, а `TitleScene` держит активность
	 * полем `wrap<title::CActivity>`. Мёртвый титул продолжал тикать
	 * (`title::CScreen@UpdateEvent`) и каждый кадр воскрешал свой освобождённый
	 * парт — плёночный шум 320x180 в углу пролога.
	 *
	 * ★Освобождать надо ТОЛЬКО локальные: АРГУМЕНТЫ этих типов ОДОЛЖЕНЫ (сайт
	 * кладёт значение чтением `X_REF`, копии не делает) — см. `delete_page_vars`
	 * и ветку «делегат завершён» в vm.c. Без этого различения выход из функции
	 * снимал ссылку, которую никто не брал: сцена умирала досрочно прямо на
	 * `DG_CALL`, а следующий `DELETE` в `RunIScene` давал double free.
	 *
	 * Почему `.LOCALDELETE` в дампе не считается: это МАРКЕР дизассемблера alice,
	 * а не инструкция — в теле `RunIScene` реальный `DELETE` ровно один
	 * (`alice ain dump -C` даёт 0 DELETE против 4 в обычном дампе). Значит
	 * освобождение локальных при выходе — обязанность движка.
	 *
	 * Диагностика, которой это найдено: `XSYS4_STRUCT_WATCH=<тип структуры>`
	 * (следить за объектом по типу — по номеру слота нельзя, он переиспользуется),
	 * сообщение о `double free` печатает стек вызовов игры и тип последнего
	 * `variable_fini`, `XSYS4_UNREF_TRACE=1` — каждое освобождение.
	 */
		if (v.i == -1)
			break;
...
{
	/*
	 * ★АРГУМЕНТЫ типа `ref <интерфейс>` (AIN_IFACE, 89) — ОДОЛЖЕННЫЕ, и
	 * освобождать их при выходе нельзя: сайт кладёт значение на стек чтением
	 * (`X_REF 2`), копии не делает, и владения не передаёт. А ЛОКАЛЬНЫЕ того же
	 * типа — владеющие: они получают значение либо от `NEW`, либо присваиванием с
	 * `SP_INC`, и освободить их обязан движок (`.LOCALDELETE` в дампе alice —
	 * маркер, а не инструкция: в теле `Scenes::RunIScene` реальный `DELETE` ровно
	 * один). Аргументы у функции — первые `nr_args` переменных.
	 *
	 * Без этого различения выход из функции снимал ссылку, которую никто не брал:
	 * сцена умирала досрочно (`CAliceLogoScene@1` вызывался прямо на `DG_CALL`), а
	 * следующий же `DELETE` в `Scenes::RunIScene` давал double free.
	 */
	int nr_args = 0;
