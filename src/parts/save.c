...
static bool parts_engine_load(struct page **buffer, bool restore_globals, bool back_scene)
{
	...
	if (version >= 6) {
		...
		if (restore_globals) {
			/*
			 * ★НОЛЬ В ФАЙЛЕ НЕ ВОССТАНАВЛИВАЕМ. Флаг нужен ровно для обратного —
			 * ВЕРНУТЬ ввод, который игра выключила перед загрузкой (см. писателя
			 * выше). Если же в сейв попал ноль (сохранение шло в момент, когда ввод
			 * был уже выключен — например, из диалога подтверждения), то восстановив
			 * его буквально, мы получаем мёртвый интерфейс: мир жив и ждёт клика, а
			 * клики не доходят, `PE_UpdateInputState` выходит на первой строке.
			 * Так и выглядел отчёт пользователя: «сохранился, потом загрузился —
			 * открылся диалог подтверждения, и ни на что нельзя кликнуть»; проверено
			 * дампом по SIGUSR1 на его прогоне (печать не вышла, потому что стояла за
			 * этим гейтом).
			 * Ноль осмыслен только для resume-пути, где игра продолжит с момента
			 * сохранения и выключит ввод сама, — но там образ и не грузится этой
			 * ветвью. Поэтому: ноль → включаем и говорим об этом в лог.
			 */
			if (!enable_input) {
				WARNING("parts-сейв: ввод записан выключенным — включаю, иначе после"
				        " загрузки интерфейс не отвечает");
				enable_input = true;
			}
