...
static struct parts_message_window *mw_require(int parts_no, const char *fn)
{
	...
	if (!mw) {
		...
		if (!warned) {
			warned = true;
			/*
			 * ★РАЗЛИЧАЕМ ДВА РАЗНЫХ ДЕФЕКТА. Одно и то же сообщение раньше
			 * покрывало и «части нет вовсе» (её снесли со слоем, а игра шлёт
			 * текст в старый номер — §5ch), и «часть есть, но загрузчик не
			 * опознал в ней окно реплик». Лечатся они в разных местах, а на
			 * экране выглядят одинаково: имя говорящего рисуется, окна с
			 * текстом нет.
			 */
			struct parts *p = parts_try_get(parts_no);
			if (!p) {
				WARNING("%s: части %d НЕТ (снесена вместе со слоем?) — "
					"текст реплики выводить некуда", fn, parts_no);
			} else {
				// Имя активности — то, что даёт найти сам узел в раскладке
				// (.pactex) и посмотреть его パーツタイプ; без него остаётся
				// только гадать, какой из 195 файлов смотреть.
				const char *act = pe_parts_activity_name(parts_no);
				WARNING("%s: часть %d ЕСТЬ, но это не メッセージウィンドウ: "
					"state=%d маска-построения=%d кнопка=%d act='%s' "
					"(узел раскладки не опознан загрузчиком)", fn, parts_no,
					(int)p->state, (int)p->construction_mask, (int)p->is_button,
					act ? act : "(не из активности)");
			}
		}
	}
	return mw;
