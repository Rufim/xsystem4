	...
	for (int i = 0; i < t->nr_lines; i++) {
		...
		for (int j = 0; j < line->nr_chars; j++, idx++) {
			...
			if (rb && rb->first_char + rb->nr_chars - 1 == idx && rb->nr_glyphs) {
				...
				float rh = text_style_height(&t->ruby_ts) * sy;
				float rx = ruby_start_x + (span - rb->width * sx) / 2.0f;
				float ry = y - rh - t->ruby_line_space * sy;
				for (int k = 0; k < rb->nr_glyphs; k++) {
					struct parts_text_char *g = &rb->glyphs[k];
					mat4 rt = WORLD_TRANSFORM(g->t.w * sx, g->t.h * sy, rx, ry);
...
void parts_render(struct parts *parts)
{
	...
	/*
	 * ★Часть, служащая альфа-МАСКОЙ, РИСУЕТСЯ КАК ОБЫЧНАЯ. «Маска» — это не отдельный
	 * вид части: `SetPartsAlphaClipperPartsNumber` лишь говорит, чьей альфой обрезать
	 * ДРУГУЮ часть, а видимость самой маски по-прежнему решает её собственный `show`.
	 *
	 * Раньше маску пропускали безусловно (d820161, под клиппер обучения Dohna), и на
	 * этом пропало слайд-шоу титула Haha Ranman: фотографии за логотипом — это часть
	 * `1000001056`, которая ОДНОВРЕМЕННО служит маской для плёночного фильтра и шума
	 * (в дампе `isclip=1`, а на неё ссылаются `clip=1000001056` у двух соседей).
	 * Поверхность у неё собиралась правильно (XSYS4_CP_DUMP: фото + серый фильтр),
	 * прозрачность игра плавно вела 0→243, — но рендер часть пропускал, и на полотне
	 * оставался фон. Замер по шести кадрам через 5 с: у оригинала полотно меняется на
	 * 24000–103000 px, у нас было 6500 (только плёночный шум).
	 *
	 * Ручка `XSYS4_NO_CLIPPER_DRAW=1` возвращает прежнее поведение для A/B.
	 */
	if (parts->is_alpha_clipper && getenv("XSYS4_NO_CLIPPER_DRAW"))
		return;
	switch (state->type) {
