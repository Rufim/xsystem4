...
static int parts_get_sprite_z(struct parts *parts)
{
	...
	/*
	 * ★ЧАСТЬ БЕЗ СЛОЯ — АНОМАЛИЯ, А НЕ РЕЖИМ. Порядок вывода задаёт позиция слоя
	 * в стеке, и для части, чей слой снят, правильного ключа НЕ СУЩЕСТВУЕТ: 0 —
	 * это ключ самого нижнего слоя, поэтому такая часть не «уходит под всё», а
	 * всплывает НАД ним (вторичный ключ — её собственный z, у остатков боя это
	 * были сотни тысяч против единиц у ADV-слоя). Отрицательный ключ не годится
	 * тем более: спрайт-сцена его не принимает, и часть пропадает совсем — так
	 * при опыте с честным сносом слоя пролог становился ЧЁРНЫМ.
	 *
	 * Штатно этого случая теперь нет: `PE_RemoveController` сносит ВСЕ части
	 * своего слоя (исключение для активностей снято, см. там же). Поэтому вместо
	 * тихой сортировки по заведомо неверному ключу — одноразовое предупреждение
	 * с номером части: если случай всё же возникнет (например, рассинхрон стека
	 * слоёв после возобновления из образа VM), он будет виден сразу, а не через
	 * «на экране лишнее» неделю спустя.
	 */
	if (unlikely(pos < 0)) {
		static bool warned = false;
		if (!warned) {
			warned = true;
			WARNING("часть %d числится за слоем %d, которого нет в стеке (глубина %d): "
				"порядок вывода для неё не определён — кладу на уровень нижнего слоя",
				parts->no, parts->controller_no, ctrl_stack.nr_controllers);
		}
		return 0;
	}
	return pos;
...
void PE_RemoveController(struct page **erase_number_list, int index)
{
	...
	bool keep_act = getenv("XSYS4_KEEP_ACT_PARTS");
	int released = 0;
	const char *sample = "";
